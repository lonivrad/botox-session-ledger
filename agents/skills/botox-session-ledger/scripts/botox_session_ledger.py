...
def build_ledger(
    ...
    output.append(f"Product: {product_clean}")
    output.append(f"Vial assumption: {VIAL_UNITS} units")
    output.append(f"Diluent: {format_number(diluent_ml)} mL")
    output.append(f"Concentration: {format_number(concentration)} units/mL")
    output.append("")
    output.append("Dilution arithmetic:")
    output.append(f"{VIAL_UNITS} units ÷ {format_number(diluent_ml)} mL = {format_number(concentration)} units/mL")
    output.append("")
    ...
    for entry in entries:
        arithmetic = f"{format_number(entry.units)} ÷ {format_number(concentration)}"
        volume = f"{entry.volume_ml:.2f} mL / {entry.volume_ml:.2f} cc"
        u100 = f"{entry.u100_markings:.0f}"
        output.append(f"| {entry.area} | {format_number(entry.units)} | {arithmetic} | {volume} | {u100} |")

    output.append("")
    output.append("Treatment summary:")
    output.append("")
    output.append(f"Total planned dose: {format_number(total_units)} units")
    output.append("")
    output.append(f"Total volume: {total_volume_ml:.2f} mL / {total_volume_ml:.2f} cc")
    output.append("")
    output.append(f"Total U-100 syringe markings: {total_u100_markings:.0f}")
    output.append("")
    output.append(f"Expected vial remaining: {format_number(expected_remaining_units)} units")
    output.append("")
    output.append(f"Vial used: {vial_percent_used * 100:.1f}%")
    output.append("")
